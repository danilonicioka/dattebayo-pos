...
# Load variables for script usage (exporting them so docker compose picks them up if needed, 
# though --env-file usually handles it for the compose command itself, we need them for echo/curl)
set -a
source "$ENV_FILE"
set +a

# Fallbacks if not in .env (though they should be)
APP_PORT="${APP_PORT:-8080}"
POSTGRES_PORT="${POSTGRES_PORT:-5433}"
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-dattebayo-dev}"

echo "🍽️  Starting Restaurant POS System (DEV)..."
...
for i in {1..30}; do
    if curl -s http://localhost:$APP_PORT > /dev/null 2>&1; then
        ...
        echo "🔄 Restart backend: ./scripts/start-dev.sh"
        exit 0
    fi
    echo -n "."
    sleep 2
